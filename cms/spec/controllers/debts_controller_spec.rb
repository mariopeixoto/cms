...
RSpec.describe DebtsController, :type => :controller do
  ...
  describe "POST #create" do
    ...
    it "creates the debt successfully when valid params are provided" do
      user = create :user
      sign_in user
    ...
    it "does not create the debt when there are validation errors" do
      user = create :user
      sign_in user
  ...
  describe "PUT #update" do
    ...
    it "creates the debt successfully when valid params are provided" do
      ...
      put :update, id: debt1.id, debt: debt_new.attributes

      expect(assigns(:debt)).to be_a(Debt)
      expect(assigns(:debt)).to be_persisted
      expect(assigns(:debt)).to have_attributes(:original_amount => debt_new.original_amount)
    end

    it "does not update the debt when there are validation errors" do
      debt1 = create(:debt)
      user = create :user
      sign_in user

      put :update, id: debt1.id, debt: {debt_type: ""}
